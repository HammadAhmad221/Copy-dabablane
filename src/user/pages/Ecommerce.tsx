    ...
    const fetchBlanes = async () => {
      try {
        ...
        if (response.data && Array.isArray(response.data)) {
          // Filter only blanes with type "order"
          const orderBlanes = response.data
            .filter((blane: ApiBlane) => blane.type === 'order')
            .map((blane: ApiBlane) => {
  ...
  return (
    <div className="min-h-screen p-4 md:p-6 lg:p-8">
      <div className="mb-6 md:mb-10">
        <h1 className="text-3xl md:text-4xl font-bold text-gray-900">Boutique en Ligne</h1>
        <p className="text-gray-600 mt-2">Découvrez nos produits disponibles à l'achat</p>
      </div>
